...
export const PanoramaViewer: React.FC<PanoramaViewerProps> = ({ id }) => {
  ...
  return (
    <div className="absolute inset-0 h-full w-full">
      ...
      {loading && !error && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 bg-[#0a0a0a] text-white">
          <svg className="w-7 h-7 animate-spin text-slate-600" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <p className="text-[13px] font-medium text-slate-400 tracking-wide">Loading 3D scene...</p>
        </div>
      )}
      {error && (
